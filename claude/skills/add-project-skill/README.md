...
| **content-synthesize** | Process raw notes into target docs | Input folder, target files and routing rules, source citation format, actionable item handling, processed note disposition (move/mark/delete) |
...
| **update-knowledge** | Persist stable decisions and learnings to KNOWLEDGE.md | Knowledge categories (decisions, learnings, conventions, ops), capture strictness, staleness checking, follow-up prompt |
| **update-session-log** | Capture session context for cross-session continuity | Log path, entry format (standard/minimal/detailed), git grounding on/off, follow-up prompt |
| **wrap-day** | End-of-day close-out with daily notes | Daily note path and template, section types (checklist, freeform, time-entry), carryover behavior, follow-up prompt |
| **wrap-session** | End-of-session wrap-up | Session log on/off, knowledge update policy, commit mode (suggest/ask/skip), git policy, extra steps (tests, lint) |
