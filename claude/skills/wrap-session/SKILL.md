...
Follow the full `/update-session-log` process:
...
- Technical gotchas or workarounds discovered
- Operational facts needed repeatedly

**Skip when:**
- Session was routine implementation with no new insights
- All learnings are session-specific, not reusable

If updating, follow the `/update-knowledge` process: read `.claude/KNOWLEDGE.md`, add/revise entries, remove stale ones, and show what changed.

### Step 3: Suggest a Commit

Follow the `/commit` process (suggest mode — no `--run`):

1. Run `git status`, `git diff`, `git diff --staged`, `git log --oneline -5`
2. Analyze changes — identify what's part of the session's work vs unrelated (e.g., runtime theme changes from theme-select)
3. Write commit message to `commit-msg.txt`
4. List which files to stage (call out any files to exclude)
5. Remind user to commit — never run `git commit` directly
